...
<section>
    <b-row class="mb-1">
        <b-col>
            <h6><strong>2.메뉴 및 컨텐츠 관리 >	Introduction > 컨텐츠 (관리)</strong></h6>
...
module.exports = {
    ...
    methods: {
        ...
        getList: async function () {
            let url = `${this.api_url}/introduction?menu_id=${this.menu_id}`;
        ...
        goForm: function (event, item) {
            if (item) {
                this.$router.push({ name: 'introduction_form', query: {menu_id: this.menu_id, id: item.id}});
            } else {
                this.$router.push({ name: 'introduction_form', query: {menu_id: this.menu_id}});
            }
        },
        deleteData: async function (event, item) {
            console.log(item);
            let url = `${this.api_url}/introduction/${item.id}`;
