...
<section>
    <b-row class="mb-1">
        <b-col>
            <h6><strong>2.메뉴 및 컨텐츠 관리 >	Map > 컨텐츠 (관리)</strong></h6>
...
module.exports = {
    ...
    methods: {
        ...
        goForm: function (event, item) {
            if (item) {
                this.$router.push({ name: 'map_form', query: {menu_id: this.menu_id, id: item.id}});
            } else {
                this.$router.push({ name: 'map_form', query: {menu_id: this.menu_id}});
            }
        },
