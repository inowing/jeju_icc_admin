      ...
      <b-col>
        <b-table small :fields="fields" :items="items" responsive="sm" class="text-center">
          <template #cell(manageBtn)="row">
            ...
            <b-button size="sm" pill variant="outline-danger" @click="loginInfo(row.item, row.index, $event.target)">
              매칭 건수
            </b-button>
          </template>
...
module.exports = {
  ...
  data: function () {
    ...
    return {
      ...
      fields: [
        ...
        {key: 'email', label: '신청 아이디'},
        {key: 'opponentCompany', label: '상담 대상 기업'},
        {key: 'date', label: '상담 신청 시간'},
      ],
      items: [
  ...
  methods: {
    ...
    getData: async function () { // 데이터 가져오기
      let url = `${this.api_url}/front/bm_statistic/get_meeting_statistic_list_seller_detail?company_id=${this.company_id}&event_id=${this.event_id}&page=${this.currentPage}&limit=${this.perPage}&attend_type=${this.attendType}&date_from=${this.dateFrom}&date_to=${this.dateTo}&search=${this.search}&status=${this.status}`;
      ...
      for (var j = 0; j < items.length; j++) {
        ...
        new_object[j] = {
          ...
          opponentCompany: data[j]['opponent_company_name'],
          date: data[j]['date'] + ' ' + data[j]['time_start'] + '-' + data[j]['time_end'],
        };
      }
    ...
    excelDownload() {
      window.location.href = `${this.api_url}/front/bm_statistic/get_meeting_statistic_list_seller_detail_excel?company_id=${this.company_id}&event_id=${this.event_id}&page=${this.currentPage}&limit=${this.perPage}&attend_type=${this.attendType}&date_from=${this.dateFrom}&date_to=${this.dateTo}&search=${this.search}&status=${this.status}`;
    }
  },
  watch: {
    currentPage: {
