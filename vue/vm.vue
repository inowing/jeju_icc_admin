<template>
  <section>
                    ...
                    <b-row>
                      <b-col cols="2" class="text-center vm_card_v_left">
                        <div>
                          ...
                          <span
                            style="font-size:smaller;">({{ week[(new Date(Date.parse(item.date.split(' ')[0]))).getDay()] }})</span><br>
                        ...
                        <b-row style="min-height: 90px;">
                          ...
                          <b-col cols="3" class="p-1">
                            <p class="text-secondary"
                              style="font-size:smaller; width: 100%; height: 100%; padding: 5px; border-left:1px solid silver; line-height: 85px; margin-bottom:0px;">
                              ...
                              <b-row>
                                <b-col align="center">
                                  <p>
                                    ...
                                    예상 참가자수 <strong class="text-primary">{{ item.event_size }}</strong>
                                    명
                                  </p>
                                  <p class="mb-1">
                                    <b-button size="sm" variant="danger" class="inoBtn-200">
                                      <b-icon-stop-circle>
                                      </b-icon-stop-circle> BroadCast 멈추기
                                    </b-button>
                                  </p>
                                  <p class="mb-1">
                        ...
                        <b-row class="text-right mb-1">
                          <b-col>
                            <b-button v-b-toggle="'toggle_'+item.id" size="sm" variant="outline-primary"
                              ...
                              <b-icon-chevron-down v-show="!openId.includes(item.id)">
                              </b-icon-chevron-down>
                              <b-icon-chevron-up v-show="openId.includes(item.id)">
                              </b-icon-chevron-up>
                            </b-button>
          ...
          <b-col>
            <b-form-group label="시간">
              <b-input-group>
                ...
                <b-input-group-append>
                  <b-form-timepicker v-model="start_time" size="sm" locale="kr" button-only>
                  </b-form-timepicker>
                </b-input-group-append>
              </b-input-group>
            </b-form-group>
        ...
        <b-row>
          <b-col>
            <b-form-group label="진행시간(분)">
              <b-form-input v-model="form.time" :state="form.time ? true: false" type="number" size="sm">
              </b-form-input>
            </b-form-group>
          </b-col>
        </b-row>
              ...
              <b-card no-body>
                ...
                <b-card-text class="mt-1 pl-1 pb-1">
                  ...
                  <b-input-group>
                    ...
                    <b-input-group-append>
                      <b-button @click="banner_file = null; form.banner=''; banner_del=true;" size="sm"
      ...
      <b-card no-body>
        <b-tabs v-model="attend_tabIndex" small card>
          <b-tab title="Operator">
            <b-table :fields="modal4_fields" :items="modal4_items_1" small bordered head-variant="light" class="mt-1"
              style="font-size: 9pt; vertical-align: center; line-height:33px;">
          ...
          <b-tab title="Moderator/Presenter">
            <b-table :fields="modal4_fields" :items="modal4_items_2" small bordered head-variant="light" class="mt-1"
              style="font-size: 9pt; vertical-align: center; line-height:33px;">
              <template #cell(id)="row">
                <div class="text-center">{{row.item.id}}</div>
              </template>
          ...
          <b-tab title="Attendee">
            <b-table :fields="modal4_fields" :items="modal4_items_3" small bordered head-variant="light" class="mt-1"
              style="font-size: 9pt; vertical-align: center; line-height:33px;">
              <template #cell(id)="row">
                <div class="text-center">{{row.item.id}}</div>
              </template>
    ...
    <b-modal v-model="modal2" hide-footer :title="modal2_type">
      ...
      <b-table v-show="modal2_type == 'Q&A'" :fields="qna_fields" :items="qna_items" small bordered head-variant="light"
        class="mt-1" style="font-size: 9pt; vertical-align: center; line-height:33px;">
        <template #cell(id)="row">
          <div class="text-center">{{row.item.id}}</div>
        </template>
      ...
      <b-table v-show="modal2_type == '참가자'" :fields="attend_fields" :items="attend_items" small bordered
        head-variant="light" class="mt-1" style="font-size: 9pt; vertical-align: center; line-height:33px;">
        <template #cell(id)="row">
          <div class="text-center">{{row.item.id}}</div>
        </template>
        <template #cell(name)="row">
...
module.exports = {
  ...
  computed: {
    ...
    availableOptions() {
      ...
      const options = this.vm.filter(
        (opt) => this.tagValue.indexOf(opt.email) === -1
      );
      if (criteria) {
        return options.filter(
          (opt) => opt.email.toLowerCase().indexOf(criteria) > -1
        ); // return new array
      }
      return options;
    },
  ...
  methods: {
    ...
    storeData: async function () {
      ...
      let date = `${this.form.date} ${
        this.start_time ? this.start_time.substr(0, 5) : "00:00"
      }`;
    ...
    openModal2: async function (item, open_type) {
      ...
      if (open_type == "Q&A") {
        let rs = await axios.get(
          `${this.api_url}/conference_qna?conference_id=${item.id}`
        );
        this.qna_items = rs.data.result;
      } else {
        let rs = await axios.get(
          `${this.api_url}/conference_log?conference_id=${item.id}`
        );
    ...
    getSelects: async function () {
      ...
      this.vm = [{ id: "a", name: "dezcao@gmail.com" }];
